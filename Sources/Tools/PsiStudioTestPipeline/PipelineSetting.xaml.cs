...
namespace PsiStudioTestPipeline
{
    ...
    public partial class PipelineSetting : Window, IPsiStudioPipeline
    {
        ...
        public void RunPipeline()
        {
            ...
            Dataset dataset = new Dataset("WebcamDD", PathName, true);
            var session = dataset.AddEmptySession("Webcam");
            // Create the webcam component
            var webcam = new MediaCapture(pipeline, 640, 480, 30);

            // Create the store component
            var store = PsiStore.Create(pipeline, "Webcam", "D:\\Stores\\Webcam");
        
            session.AddPartitionFromPsiStoreAsync("Webcam", "D:\\Stores\\Webcam");
